...
public void OnPluginStart() {
	CreateConVar("sm_vipmenu_version", PLUGIN_VERSION, "VIP menu version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	g_cvarReload = CreateConVar("sm_vipmenu_autoreload", "0", "Automatically reload the configuration file when changing the map.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvarConfigPath = CreateConVar("sm_vipmenu_config_path", "configs/vipmenu.cfg", "Path to configuration file.");
...
public Action Command_VipMenuReload(int client, int args) {
	char hc[PLATFORM_MAX_PATH];
	char buffer[PLATFORM_MAX_PATH];
...
bool CommandExist(char[] command, int commandLen) {
